...
trait BoidSequence:
  ...
  /** Adds a boid to the front of the sequence.
    *
    * @param boid
    *   the boid to add
    *
    * @return
    *   a new sequence of boids with the given boid added to the front
    */
  def add(boid: Boid): BoidSequence = BoidCons(boid, this)

  /** Converts boid sequence to a list.
    *
    * @return
    *   a list of boids
    */

  def toList: List[Boid] = this match
    case BoidNil()            => List.empty
    case BoidCons(head, tail) => head :: tail.toList

  /** Checks whether the sequence of boids contains the given boid.
    *
    * @param boid
    *   the boid to check for
    *
    * @return
    *   `true` if the sequence contains the given boid, else `false`
    */
  def contains(boid: Boid): Boolean = this match
    case BoidNil()            => false
    case BoidCons(head, tail) => head == boid || tail.contains(boid)

  /*  Extends the BoidSequence by a Boid using operator ++.
    *
    * @param boid
    *   the Boid to add
    *
    * @return
    *   a new BoidSequence with the given Boid added
    */
  def ++(boids: BoidSequence): BoidSequence = this match
    case BoidNil()           => boids
    case BoidCons(h, t)      => BoidCons(h, t ++ boids)
  


  /** Convert BoidSequence to a sequence of boids.
    *
    * @return
    *   a sequence of boids
    */
  def toSeq: Seq[Boid] = this match
    case BoidNil()            => Seq.empty
    case BoidCons(head, tail) => head +: tail.toSeq

  /* Reverse the BoidSequence.
    *
    * @return
    *   a new BoidSequence with the elements in reverse order
    */
  def reverse: BoidSequence = this match
    case BoidNil()            => BoidNil()
    case BoidCons(head, tail) => tail.reverse ++ BoidCons(head, BoidNil())

  /* Convert sequence to BoidSequence.
    *
    * @param seq
    ...
    *   a BoidSequence
    */
  def fromSeq(seq: Seq[Boid]): BoidSequence = seq match
    case Seq()     => BoidNil()
    case boid +: t => BoidCons(boid, fromSeq(t))
